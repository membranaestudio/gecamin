...
const
  path = {
    scripts: {
      ...
      components: [
        compilation.src + '/components/**/*.js',
      ],
      app: [
        compilation.src + '/static/js/app.js',
      ],
    },
    styles: {
      components: [
...
function JScomponents() {
  return src(path.scripts.components, {
    allowEmpty: true
  })
    .pipe(plumber())
    .pipe(dest(`${compilation.dist}/js/components`))
    .pipe(server.reload({
      stream: true
    }));
}
